...
namespace Booble_IA_API._3___Repository
{
    public class UsuarioRepository : IUsuarioRepository
    {
        ...
        public async Task<bool> Cadastro(UsuarioDTO usuario)
        {
            try
            {
                Usuario usuarioInsert = new Usuario
                {
                    ...
                    Flg_Sexo = (bool)usuario.Flg_Sexo,
        ...
        public async Task<Usuario> Login(UsuarioDTO loginRequest)
        {
            try
            {
                var usuario = await _boobleContext.Usuarios.FirstOrDefaultAsync(u => u.Des_Email == loginRequest.Des_Email.ToLower());

                if (usuario == null)
                    throw new Exception("Usuário não encontrado, verifique as credenciais e tente novamente");
